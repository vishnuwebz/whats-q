...
export const BulkRecipientListsView: React.FC<BulkRecipientListsViewProps> = ({ initialViewMode }) => {
  ...
  const [isSuppressionDetailsModalOpen, setIsSuppressionDetailsModalOpen] = useState(false);
  const [suppressionModalInitialTab, setSuppressionModalInitialTab] = useState<'all' | 'blocked' | 'opted_out' | 'quality'>('all');

  const handleOpenSuppressionDetails = (tab: 'all' | 'blocked' | 'opted_out' | 'quality') => {
    if (tab === 'all') setSuppressionFilter('all');
    else if (tab === 'blocked') setSuppressionFilter('blocked');
    else if (tab === 'opted_out') setSuppressionFilter('opted_out');
    setSuppressionModalInitialTab(tab);
    setIsSuppressionDetailsModalOpen(true);
  };
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#F8FAFC] overflow-y-auto">
      ...
      <div className="max-w-7xl mx-auto w-full p-6 space-y-5">
        {viewMode === 'suppression' ? (
          <div className="space-y-5 animate-in fade-in duration-200">
            {/* 4 Metric Summary Cards - Click any option to open details & diagnostics */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {/* Option 1: Total Suppressed */}
              <div
                role="button"
                tabIndex={0}
                onClick={() => handleOpenSuppressionDetails('all')}
                onKeyDown={(e) => e.key === 'Enter' && handleOpenSuppressionDetails('all')}
                className={`bg-white rounded-2xl border p-4 shadow-xs hover:shadow-md transition-all cursor-pointer group text-left relative overflow-hidden select-none active:scale-[0.99] ${
                  suppressionFilter === 'all'
                    ? 'border-rose-400 ring-2 ring-rose-500/20'
                    : 'border-slate-200/90 hover:border-rose-300'
                }`}
                title="Click to view full suppression intelligence & contacts"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="p-2 rounded-xl bg-rose-50 text-rose-600 group-hover:bg-rose-100 group-hover:scale-110 transition-all">
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold text-rose-600 group-hover:text-rose-700">
                  <span>View Breakdown &amp; Details</span>
                  <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>

              {/* Option 2: Blocked (Meta 131051) */}
              <div
                role="button"
                tabIndex={0}
                onClick={() => handleOpenSuppressionDetails('blocked')}
                onKeyDown={(e) => e.key === 'Enter' && handleOpenSuppressionDetails('blocked')}
                className={`bg-white rounded-2xl border p-4 shadow-xs hover:shadow-md transition-all cursor-pointer group text-left relative overflow-hidden select-none active:scale-[0.99] ${
                  suppressionFilter === 'blocked'
                    ? 'border-red-400 ring-2 ring-red-500/20'
                    : 'border-slate-200/90 hover:border-red-300'
                }`}
                title="Click to inspect Meta Error 131051 webhook telemetry & blocked contacts"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider group-hover:text-red-700 transition-colors">
                    Blocked (Meta 131051)
                  </span>
                  <div className="p-2 rounded-xl bg-red-50 text-red-600 group-hover:bg-red-100 group-hover:scale-110 transition-all">
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold text-red-600 group-hover:text-red-700">
                  <span>Inspect Webhook Telemetry</span>
                  <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>

              {/* Option 3: Opted Out / STOP */}
              <div
                role="button"
                tabIndex={0}
                onClick={() => handleOpenSuppressionDetails('opted_out')}
                onKeyDown={(e) => e.key === 'Enter' && handleOpenSuppressionDetails('opted_out')}
                className={`bg-white rounded-2xl border p-4 shadow-xs hover:shadow-md transition-all cursor-pointer group text-left relative overflow-hidden select-none active:scale-[0.99] ${
                  suppressionFilter === 'opted_out'
                    ? 'border-amber-400 ring-2 ring-amber-500/20'
                    : 'border-slate-200/90 hover:border-amber-300'
                }`}
                title="Click to view keyword STOP and button opt-out compliance details"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider group-hover:text-amber-700 transition-colors">
                    Opted Out / STOP
                  </span>
                  <div className="p-2 rounded-xl bg-amber-50 text-amber-600 group-hover:bg-amber-100 group-hover:scale-110 transition-all">
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold text-amber-600 group-hover:text-amber-700">
                  <span>View Opt-Out Compliance</span>
                  <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>

              {/* Option 4: Sender Quality */}
              <div
                role="button"
                tabIndex={0}
                onClick={() => handleOpenSuppressionDetails('quality')}
                onKeyDown={(e) => e.key === 'Enter' && handleOpenSuppressionDetails('quality')}
                className="bg-white rounded-2xl border border-slate-200/90 hover:border-emerald-300 p-4 shadow-xs hover:shadow-md transition-all cursor-pointer group text-left relative overflow-hidden select-none active:scale-[0.99]"
                title="Click to view WhatsApp Sender Quality, tier limits & health scorecard"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider group-hover:text-emerald-700 transition-colors">
                    Sender Quality
                  </span>
                  <div className="p-2 rounded-xl bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100 group-hover:scale-110 transition-all">
                ...
                <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] font-bold text-emerald-600 group-hover:text-emerald-700">
                  <span>WABA Health Scorecard (98/100)</span>
                  <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-4 flex flex-col md:flex-row items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => {
                    const tab = suppressionFilter === 'blocked' ? 'blocked' : suppressionFilter === 'opted_out' ? 'opted_out' : 'all';
                    handleOpenSuppressionDetails(tab);
                  }}
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-rose-200 bg-rose-50 text-xs font-bold text-rose-700 hover:bg-rose-100 transition cursor-pointer"
                >
                  <ShieldAlert className="w-3.5 h-3.5 text-rose-600" />
                  <span>Diagnostics &amp; Intel</span>
                </button>
      ...
      {/* Suppression Details & Intelligence Modal */}
      <SuppressionDetailsModal
        isOpen={isSuppressionDetailsModalOpen}
        onClose={() => setIsSuppressionDetailsModalOpen(false)}
        initialTab={suppressionModalInitialTab}
        onFilterChange={(tab) => setSuppressionFilter(tab)}
      />
