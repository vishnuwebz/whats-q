...
interface Props {
  ...
  onDeleteStructure?: (id: string | number) => void;
...
export const ManageSalaryView: React.FC<Props> = ({
  ...
  const handleSaveStructure = (struct: SalaryStructure) => {
    if (editingStructure) {
      onUpdateStructure(struct);
      if (selectedStructurePreview === editingStructure.name) {
        setSelectedStructurePreview(struct.name);
      }
      showToast(`Salary structure "${struct.name}" updated successfully!`);
    } else {
      onAddStructure(struct);
      setSelectedStructurePreview(struct.name);
      showToast(`Salary structure "${struct.name}" created successfully!`);
    }
    setIsAddModalOpen(false);
    setEditingStructure(null);
  };

  const handleDuplicateStructure = (s: SalaryStructure) => {
    ...
    onAddStructure(duplicated);
    setSelectedStructurePreview(duplicated.name);
    showToast(`Structure "${s.name}" duplicated as "${duplicated.name}"!`);
  };

  const handleDeleteStructure = (id: number | string) => {
    const struct = structures.find((s) => s.id === id);
    if (!struct) return;
    if (struct.employees_count > 0) {
      showToast(`Cannot delete "${struct.name}" because ${struct.employees_count} employees are currently assigned to it.`);
      return;
    }
    if (onDeleteStructure) {
      onDeleteStructure(id);
    }
    if (selectedStructurePreview === struct.name) {
      const remaining = structures.filter((s) => s.id !== id);
      if (remaining.length > 0) {
        setSelectedStructurePreview(remaining[0].name);
      }
    }
    showToast(`Salary structure "${struct.name}" deleted.`);
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {salaryComponents.map((c, i) => (
                    <tr key={c.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3 px-3 text-right">
                        <button
                          onClick={() => setEditingMasterComp(c)}
      ...
      {editingMasterComp && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg p-5 sm:p-6 space-y-4 text-xs animate-in zoom-in-95 duration-150">
            <div className="flex items-start justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-bold text-base text-slate-900">Edit Component: {editingMasterComp.name}</h3>
                <p className="text-[11px] text-slate-500">Update calculation formula, taxability, and classification.</p>
              </div>
              <button
                onClick={() => setEditingMasterComp(null)}
                className="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-100 cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                setSalaryComponents((prev) =>
                  prev.map((c) => (c.id === editingMasterComp.id ? editingMasterComp : c))
                );
                showToast(`Component "${editingMasterComp.name}" updated successfully!`);
                setEditingMasterComp(null);
              }}
              className="space-y-4"
            >
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  required
                  value={editingMasterComp.name}
                  onChange={(e) => setEditingMasterComp({ ...editingMasterComp, name: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="font-bold text-slate-700">Component Type *</label>
                  <select
                    value={editingMasterComp.type}
                    onChange={(e) => setEditingMasterComp({ ...editingMasterComp, type: e.target.value as any })}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 outline-none"
                  >
                    <option value="Earning">Earning</option>
                    <option value="Deduction">Deduction</option>
                ...
                <div className="space-y-1">
                  <label className="font-bold text-slate-700">Calculation Method *</label>
                  <select
                    value={editingMasterComp.calcType}
                    onChange={(e) => setEditingMasterComp({ ...editingMasterComp, calcType: e.target.value as any })}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 outline-none"
                  >
                    <option value="Fixed %">Fixed % of CTC</option>
                    <option value="Fixed Amount">Fixed Monthly Amount</option>
              ...
              <div className="space-y-1">
                <label className="font-bold text-slate-700">Formula / Standard Value *</label>
                <input
                  type="text"
                  required
                  value={editingMasterComp.value}
                  onChange={(e) => setEditingMasterComp({ ...editingMasterComp, value: e.target.value })}
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={editingMasterComp.description}
                  onChange={(e) => setEditingMasterComp({ ...editingMasterComp, description: e.target.value })}
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between">
                <div>
                  <span className="font-bold text-slate-800 block">Tax Treatment</span>
                  <span className="text-[10px] text-slate-500">Consider for TDS / Income Tax calculation</span>
                </div>
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={editingMasterComp.taxable}
                    onChange={(e) => setEditingMasterComp({ ...editingMasterComp, taxable: e.target.checked })}
                    className="rounded text-emerald-600 accent-emerald-600"
                  />
                  <span className="font-bold text-slate-800 text-xs">Taxable</span>
                </label>
      ...
      {/* Salary Structure Modal (Add / Edit) */}
